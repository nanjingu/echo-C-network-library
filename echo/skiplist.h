...
void SkipList<K, V>::get_data_from_string(const std::string& str, std::string* key, std::string* value)
{
    if(!is_valid_string(str))
        return;
    *key = str.substr(0, str.find(sl_colon));
    *value = str.substr(str.find(sl_colon)+1, str.length());
}
...
bool SkipList<K, V>::is_valid_string(const std::string& str)
{
    if(str.empty())
        return false;
    if(str.find(sl_colon) == std::string::npos)
        return false;
    return true;
}
...
std::string SkipList<K, V>::search_element(K key)
{
    ...
    {
        //std::cout<<"get key :"<< current->get_key() << "serach key :" << key << ":" << std::endl;
